...
class VIT_MODEL(nn.Module):
    ...
    def forward(self, image):
        #output = self.vit(image, output_attentions=True, return_dict=True)
        #res = {'embeddings': output['last_hidden_state'][:, 1:, :], 'cls': output['pooler_output'], 'attentions': output['attentions']}
...
def get_vision_model(args):
    ...
    if args.vision_backbone == 'vit':
        if args.vision_model == 'base': vit = ViTModel.from_pretrained(model_path)
...
def get_vision_configuration(args, model):
    optimizer = optim.AdamW(model.parameters(), lr=args.vision_lr, weight_decay=args.vision_weight_decay)
